<x-sidebarAdmin>
    ...
    <div class="flex justify-center items-center w-full px-4 py-8">
      <form method="POST" action="{{url('user/update/'.$expo)}}"
            class="bg-white shadow-soft-xl rounded-2xl p-6 w-full max-w-md mx-auto flex flex-col items-center">
        @csrf
        @method('PUT')
        
        <h6 class="mb-6 font-bold text-lg text-slate-700 text-center">Editar Usuario</h6>
        ...
        <div class="mb-4 w-full flex flex-row justify-center gap-4">
          <div class="flex flex-col items-center w-1/2">
            ...
            <input type="text" id="nombre" name="nombre" required
                   ...
                   placeholder="Jon Doe" value="{{$user->name}}">
          </div>
        ...
        <div class="mb-4 w-full flex flex-row justify-center gap-4">
          ...
          <div class="flex flex-col items-center w-1/2">
            ...
            <input type="text" id="correo" name="correo" required
                   class="max-w-sm w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-green-400 focus:outline-none focus:shadow-soft-primary-outline transition-all"
                   placeholder="ejemplo@correo.com" value="{{$user->email}}">
          </div>
        </div>
        ...
        <div class="mb-4 w-full flex flex-row justify-center gap-4">
          <div class="flex flex-col items-center w-1/2">
            <label for="telefono" class="block mb-2 text-sm font-semibold text-slate-600 text-center"></label>
            <input type="hidden" id="pass" name="pass" required
                   class="max-w-sm w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-green-400 focus:outline-none focus:shadow-soft-primary-outline transition-all"
                   placeholder="123Abc" value="{{$user->password}}">
          </div>
       ...
       <div class="mb-6 w-full flex flex-row justify-center gap-4">
            <div class="flex flex-col items-center w-1/2">
                ...
                <select id="rol" name="rol" 
                        class="max-w-sm w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-green-400 focus:outline-none focus:shadow-soft-primary-outline transition-all">
                <option value="{{$user->rol_id}}"  selected>{{$roln}} 
